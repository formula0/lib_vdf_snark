...
#[derive(Debug, Clone)]
pub struct PoE<'a> {
   pub inputs: Option<PoEInputs<'a>>,
   pub params: PoEParams,
}
   ...
   -> Result<Parameters<E>, SynthesisError>    //
{
   ...
   //let params: Result<Parameters<E>, SynthesisError> = generate_parameters(poe, rng);
   ...
   R: Rng,
{
   ...
   //let proof = prover.create_random_proof(params, rng);
   Ok(proof)
}
...
/*
    ...
    testPoe.clone().synthesize(&mut cs).expect("Synthesis Failed");
    println!("circuit is satisfied : {}", cs.is_satisfied());
*/
/*
